...
namespace CNum::Model::Tree {
  ...
  // ---- Preperation for exact greedy tree build ----
  ...
  // ---- Preperation for histogram tree build ----
  void XGTreeBooster::fit_prep(const Matrix<int> &X,
    ...
    double gs = ::std::reduce(::std::execution::seq, g + partition.start, g + partition.end);
    double hs = ::std::reduce(::std::execution::seq, h + partition.start, h + partition.end);
